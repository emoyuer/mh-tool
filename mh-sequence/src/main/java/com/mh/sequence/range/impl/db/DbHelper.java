...
abstract class DbHelper {
    ...
    private final static String SQL_CREATE_TABLE =
            ...
            + "gmt_modified DATETIME NOT NULL,"+ "gmt_date date NOT NULL," + "PRIMARY KEY (`id`),UNIQUE uk_name (`name`,`gmt_date`)" + ")";
    private final static String SQL_INSERT_RANGE =
            "INSERT IGNORE INTO #tableName(name,value,gmt_create,gmt_modified,gmt_date)" + " VALUE(?,?,?,?,?)";
    private final static String SQL_UPDATE_RANGE =
            "UPDATE #tableName SET value=?,gmt_modified=? WHERE name=? AND " + "value=?";
    private final static String SQL_MYUPDATE_RANGE =
            "UPDATE #tableName SET value=?,gmt_modified=? WHERE name=? AND " + "value=? AND " + "gmt_date=?";
    private final static String SQL_SELECT_RANGE = "SELECT value,gmt_create FROM #tableName WHERE name=?";
    private final static String SQL_MYSELECT_RANGE = "SELECT value,gmt_create FROM #tableName WHERE name=? AND gmt_date=?";
     ...
     * @return 成功/失败
     */
    static boolean myUpdateRange(DataSource dataSource, String tableName, Long newValue, Long oldValue, String name) {
        ...
        try {
            conn = dataSource.getConnection();
            stmt = conn.prepareStatement(SQL_MYUPDATE_RANGE.replace("#tableName", tableName));
            stmt.setLong(1, newValue);
            stmt.setTimestamp(2, new Timestamp(System.currentTimeMillis()));
            stmt.setString(3, name);
            stmt.setLong(4, oldValue);
     ...
     * @return 区间值
     */
    static Long mySelectRange(DataSource dataSource, String tableName, String name, long stepStart) {
        Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        long oldValue;
        ...
        try {
            conn = dataSource.getConnection();
            stmt = conn.prepareStatement(SQL_MYSELECT_RANGE.replace("#tableName", tableName));
            stmt.setString(1, name);
            stmt.setDate(2, getNowDate());
            rs = stmt.executeQuery();
            if (!rs.next()) {
                //没有此类型数据，需要初始化
                insertRange(dataSource, tableName, name, stepStart);
