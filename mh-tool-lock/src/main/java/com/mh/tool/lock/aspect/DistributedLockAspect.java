package com.mh.tool.lock.aspect;

import com.mh.tool.lock.annotation.MhLock;
...
public class DistributedLockAspect {
    ...
    //切入点：有这个注解的方法
    @Pointcut("@annotation(com.mh.tool.lock.annotation.MhLock)")
    ...
    private Object startLock(final String lockKey, ProceedingJoinPoint pjp, Method method) throws Throwable {
        MhLock annotation = method.getAnnotation(MhLock.class);
         //是否尝试获得锁
    ...
    private Object lock(ProceedingJoinPoint pjp, MhLock annotation, String lockKey) throws Throwable {
        int leaseTime = annotation.leaseTime();
        TimeUnit timeUnit = annotation.timeUnit();
        try {
            if(leaseTime == -1){
                //不设置超时时间，不会自动释放。
                MhLockUtil.lock(lockKey);
            }else {
                //超时时间过后，锁自动释放。
                MhLockUtil.lock(lockKey,timeUnit,leaseTime);
            }
            return pjp.proceed();
        } finally {
            MhLockUtil.unlock(lockKey);
        }
    }


    private Object tryLock(ProceedingJoinPoint pjp, MhLock lock, String lockKey) throws Throwable {
        ...
        try {
            ...
            if (waitTime == 0) {
                ...
                MhLockUtil.tryLock(lockKey, timeUnit, waitTime, leaseTime);
                //执行业务方法
                return pjp.proceed();
            }
        } finally {
